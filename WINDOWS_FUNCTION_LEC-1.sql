...
-- Windows function = are the function which are used to perform the calculation on set of rows
-- these calculation are done in relation to the current row
-- windows function do not collapse the data like group by
-- 1. over() = it defines set of rows

create database windows10db;

drop database windows10db;
use windows10db;
...
over(partition by department) from employee_sales;

select employee_name, department, sales_amount, sum(sales_amount) 
over(partition by employee_name) from employee_sales;

select employee_name, department, sales_amount, avg(sales_amount) 
over(partition by department) as dept_average,
